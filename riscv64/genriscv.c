...
int choose_reg(Eestmt *stmt, int appoint_reg)
{
    ...
    if (isglobal == 1) // 全局变量,要存回去
    {
        ...
        fprintf(listing, "lui %c%d,%%hi(v%d)\nadd %c%d,%c%d,%%lo(v%d)\n",rc,ri,gid,rc,ri,rc,ri,gid);
        // fprintf(listing, "%c%d [0] = ", rc, ri);
        reg_change(retid);
        fprintf(listing, "sw %c%d,", rc, ri);
        // fprintf(listing, "%c%d\n", rc, ri);
        reg_change(reg1);
        fprintf(listing, "(%c%d)\n", rc, ri);
...
void get_riscv_code(stmtkind op,int reg1, int reg2, int reg3)
{
    ...
    if (op == EQ_E)
        fprintf(listing, "seqz %c%d,%c%d\n",rc,ri,rc,ri);


    if (op == NOT_E)
    {
        reg_change(reg1);
        fprintf(listing, "seqz %c%d,",rc,ri);
        reg_change(reg2);
        fprintf(listing, "%c%d\n",rc,ri);
    }
...
void genriscv(Eestmt *stmt)
{
    ...
    if (stmt->kind <= GT_E)
    {
        ...
        else if (reg1 == -1 || reg2 == -1) //有一个是数字
        {
            ...
            if (stmt->kind == PLUS_E || stmt->kind == LT_E) //可以用reg = reg op2 int 小于的话，右边一定是数字
            {
                ...
                // fprintf(listing, "%c%d = ", rc, ri);
                if(stmt->kind == LT_E)  
                    fprintf(listing, "slti %c%d,", rc, ri);
                else
                    fprintf(listing, "add %c%d,", rc, ri);
                reg_change(thereg);
            ...
            else
            {
                ...
                if (reg1 == -1)   //左数是num
                {
                    ...
                    // reg_change(thereg);
                    // fprintf(listing, "%c%d\n", rc, ri);
                    get_riscv_code(stmt->kind,reg0,reg3,thereg);
                }
                else
                {
                    // reg_change(reg0);
                    ...
                    // reg_change(reg3);
                    // fprintf(listing, "%c%d\n", rc, ri);
                    get_riscv_code(stmt->kind,reg0,thereg,reg3);
                }
            }
        }
        else
        {
            ...
            // reg_change(reg2);
            // fprintf(listing, "%c%d\n", rc, ri);
            get_riscv_code(stmt->kind,reg0,reg1,reg2);
        }
        //把设置关于reg0的状态
    ...
    else if (stmt->kind <= NEG_E)
    {
        ...
        else
        {
            ...
            // reg_change(reg1);
            // fprintf(listing, "%c%d\n", rc, ri);
            get_riscv_code(stmt->kind,reg0,reg1,-1);
        }
    ...
    else if (stmt->kind == ARRAY_DEF_E)
    {
        ...
        fprintf(listing, "add %c%d,sp,%d*4 \n", rc, ri, stackp);
    ...
    else if (stmt->kind == ASS_FROM_ARR_E)
    {
        ...
        // reg_change(reg2);
        // fprintf(listing, "%c%d\n", rc, ri); //计算偏移量
        get_riscv_code(PLUS_E,reg3,reg1,reg2);

        reg_change(reg0);
        // fprintf(listing, "%c%d = ", rc, ri);
        fprintf(listing, "lw %c%d,", rc, ri);
        reg_change(reg3);
...
void start_optimize(Eeglobal *tree)
{
    while (tree)
    {
        if (tree->kind == 0)
        {
            ...
            // if(strcmp(tree->funname,"main") == 0)
            // {
            //     fprintf(listing,".size f, .-f\n.text\n.align 2\n.global main\n.type main, @function\nmain:\nadd sp,sp,-%d\nsw ra,%d(sp)\n",stk,stk-4);
            // }
            // else
            // {
                fprintf(listing,".text\n.align 2\n.global %s\n.type %s, @function\n%s:\nadd sp,sp,-%d\nsw ra,%d(sp)\n",tree->funname,tree->funname,tree->funname,stk,stk-4);
            //}
            ...
            fprintf(listing,".size %s, .-%s\n",tree->funname,tree->funname);
            stk = 0;
        }
        else if (tree->kind == 1)
        {
